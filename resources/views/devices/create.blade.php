...
@section('title', __('messages.add_new_device'))

@section('header')
    <h1>{{ __('messages.add_new_device') }}</h1>
    <p>{{ __('messages.configure_zkteco_device') ?? 'إعداد جهاز ZKTeco جديد' }}</p>
@endsection
    ...
    <form method="POST" action="{{ route('devices.store') }}">
        ...
        <div class="form-grid-2">
            <div class="form-group">
                ...
                <input type="text" name="name" id="name" value="{{ old('name') }}" class="form-control" placeholder="e.g. Main Entrance" required>
                @error('name') <span class="form-error">{{ $message }}</span> @enderror
            </div>

            <div class="form-group">
                <label for="location" class="form-label">{{ __('messages.location') }}</label>
                <input type="text" name="location" id="location" value="{{ old('location') }}" class="form-control" placeholder="e.g. Building A - Floor 1">
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <p class="form-hint">{{ app()->getLocale() == 'ar' ? 'اختياري: حدد الفرع التابع له هذا الجهاز' : 'Optional: Select the branch this device belongs to' }}</p>
        </div>
        @endif
        ...
        <div class="form-group">
            ...
            <div style="display: flex; gap: 12px; margin-top: 6px;">
                <label style="display: flex; align-items: center; gap: 8px; padding: 12px 20px; border: 2px solid var(--border-color); border-radius: 8px; cursor: pointer; transition: all 0.2s;" class="mode-option" id="mode-pull-label">
                    <input type="radio" name="connection_mode" value="pull" {{ old('connection_mode', 'pull') === 'pull' ? 'checked' : '' }} onchange="toggleConnectionMode()">
                    <div>
                ...
                <label style="display: flex; align-items: center; gap: 8px; padding: 12px 20px; border: 2px solid var(--border-color); border-radius: 8px; cursor: pointer; transition: all 0.2s;" class="mode-option" id="mode-push-label">
                    <input type="radio" name="connection_mode" value="push" {{ old('connection_mode') === 'push' ? 'checked' : '' }} onchange="toggleConnectionMode()">
                    <div>
        ...
        <div id="pull-fields">
            <div class="form-grid-2">
                <div class="form-group">
                    ...
                    <input type="text" name="ip_address" id="ip_address" value="{{ old('ip_address') }}" class="form-control" placeholder="e.g. 192.168.1.201">
                    @error('ip_address') <span class="form-error">{{ $message }}</span> @enderror
                </div>

                <div class="form-group">
                    <label for="port" class="form-label">{{ __('messages.port') }}</label>
                    <input type="number" name="port" id="port" value="{{ old('port', 4370) }}" class="form-control">
                    <p class="form-hint">{{ __('messages.default_port_hint') ?? 'المنفذ الافتراضي لأجهزة ZKTeco هو 4370' }}</p>
                </div>
            </div>
        </div>
        ...
        <div id="push-fields" style="display: none;">
            <div class="form-group">
                ...
                <input type="text" name="serial_number" id="serial_number" value="{{ old('serial_number') }}" class="form-control" placeholder="e.g. BFKJ203560012">
        ...
        <div class="form-actions" style="margin-top: 20px;">
            <button type="submit" class="btn btn-primary">{{ __('messages.save_device') }}</button>
